...
- (void)loadView {
	...
	homescreenController = [[%c(CBViewController) alloc] initWithCanvasServer:[%c(CBCanvasServer) sharedServer]];
...
- (void)loadView {
    %orig;
    self.view.clipsToBounds = YES;
    lockscreenController = [[%c(CBViewController) alloc] initWithCanvasServer:[%c(CBCanvasServer) sharedServer]];
